...
writer = SummaryWriter(comment='_CNN_LSTM')
...
class CNN(nn.Module):
    def __init__(self, input_channels, kernel_sizes, filters, fc_units):
        ...
        self.conv_small = nn.ModuleList()
        self.conv_large = nn.ModuleList()

        in_channels = input_channels
        for i in range(len(kernel_sizes)):
            self.conv_small.append(
                nn.Conv1d(in_channels, filters[i], kernel_size=kernel_sizes[i], padding=kernel_sizes[i] // 2))
            self.conv_small.append(nn.BatchNorm1d(filters[i]))
            self.conv_small.append(nn.ReLU())
            self.conv_small.append(nn.MaxPool1d(kernel_size=1))

            large_kernel = kernel_sizes[i] * 2
            self.conv_large.append(
                nn.Conv1d(in_channels, filters[i], kernel_size=large_kernel, padding=large_kernel // 2))
            self.conv_large.append(nn.BatchNorm1d(filters[i]))
            self.conv_large.append(nn.ReLU())
            self.conv_large.append(nn.MaxPool1d(kernel_size=1))
    ...
    def forward(self, x):
        small_out = x
        large_out = x
        for layer in self.conv_small:
            small_out = layer(small_out)
        for layer in self.conv_large:
            large_out = layer(large_out)

        small_out = F.adaptive_avg_pool1d(small_out, large_out.size(-1))

        out = small_out + large_out
